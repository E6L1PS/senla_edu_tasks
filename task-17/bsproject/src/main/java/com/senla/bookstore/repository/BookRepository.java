...
public class BookRepository extends AbstractRepository<Integer, Book> implements IBookRepository<Integer, Book> {
    ...
    public String findDescriptionById(Integer id) {
        TypedQuery<String> q = getEntityManager().createQuery("select b.description from Book b WHERE b.id=:id", String.class);
    ...
    public Integer findPriceById(Integer id) {
        TypedQuery<Integer> q = getEntityManager().createQuery("select b.price from Book b WHERE b.id=:id", Integer.class);
        q.setParameter("id", id);
        return q.getSingleResult();
    }
}
